...
apiListLADsEnd="&outFields=LSOA21CD,LSOA21NM,MSOA21CD,MSOA21NM,LAD22NM,LAD22CD&returnGeometry=false&outSR=4326&resultType=standard&f=json"
...
fields=["LAD22NM","LAD22CD","MSOA21NM","MSOA21CD","LSOA21NM","LSOA21CD"]
...
for j in range(0,len(LondonOAs["features"]),1):
        ...
        LAD_Code=LondonOAs["features"][j]["attributes"]["LAD22CD"]
        msoa_stage=pd.DataFrame(columns=fields)
        msoa_stage.loc[0]=[LAD_Name,LAD_Code,MSOA_Name,MSOA_Code,LSOA_Name,LSOA_Code]
        LondonOAsList=pd.concat([LondonOAsList,msoa_stage],axis=0)   

LondonOAsList.drop_duplicates(inplace=True)
LondonOAsList.to_csv("../assets/LondonAreas.csv",index=False)
